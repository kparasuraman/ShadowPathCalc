    ...
    routesData.forEach((data, index) => {
      if (data.features && data.features.length > 0) {
        data.features.forEach((selectedRoute, idx) => {
          ...
          // Add route to map and store it in the routes array
...
startInput.addEventListener('input', () => fetchAutocompleteSuggestions(startInput.value, startInput, startSuggestions));
endInput.addEventListener('input', () => fetchAutocompleteSuggestions(endInput.value, endInput, endSuggestions));


calculateButton.addEventListener('click', async () => {
  const startCoords = await fetchCoordinates(startInput.value);
  const endCoords = await fetchCoordinates(endInput.value);
  clearError();
  showLoading();
...
const azimuthCalculator = new Azimuth(latitudestart, longitudestart,latitudeend ,longitudeend,date, timeOfDay);
